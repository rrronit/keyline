...
export const useKeyboardEvents = ({
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'ArrowDown' && state.mode === 'result') {
        e.preventDefault();
        setSelectedIndex((prev: number) => (prev < filteredResults.length - 1 ? prev + 1 : 0));
      }

      if (e.key === 'ArrowUp' && state.mode === 'result') {
        e.preventDefault();
        setSelectedIndex((prev: number) => (prev > 0 ? prev - 1 : filteredResults.length - 1));
      }
  ...
  }, [state.mode, state.query, filteredResults, selectedIndex, setState, setSelectedIndex]);
};
