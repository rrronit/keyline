...
export const ResultList = ({ results, selectedIndex, onSelect, loading }: ResultListProps) => {
    ...
    if (listRef.current && selectedIndex >= 0) {
      ...
      if (selectedElement) {
        ...
        if (elementRect.bottom > containerRect.bottom) {
          listRef.current.scrollTop += elementRect.bottom - containerRect.bottom - 1;
        } else if (elementRect.top < containerRect.top) {
          listRef.current.scrollTop -= containerRect.top - elementRect.top;
        }
      }
    }
  }, [selectedIndex]);
  ...
  return (
    ...
    >
      {loading ? (
        ...
        <div ref={listRef} className="scrollbar max-h-96 overflow-x-hidden overflow-y-auto py-2">
